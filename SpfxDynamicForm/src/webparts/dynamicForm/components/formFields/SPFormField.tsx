...
import SPFieldButton from './SPFieldButton';
...
const DisplayFieldTypeMappings: {
  ...
  TaxonomyFieldTypeMulti: { component: SPFieldTextDisplay, valuePreProcess: (val) => val ? val.map((v: any) => v.Label).join(', ') : '' },
  Button: { component: SPFieldButton }
  /* The following are known but unsupported types as of now:
  Attachments: null,
  */
};
...
const SPFormField: React.SFC<ISPFormFieldProps> = (props) => {
  ...
  if (props.controlMode === ControlMode.Display) {
    if (DisplayFieldTypeMappings.hasOwnProperty(fieldType)) {
      ...
      fieldControl = <div className={`ard-${fieldType}field-display`}>
        ...
        <div className={styles.unsupportedFieldMessage}><Icon iconName='Error' />{` "${fieldType}"`}</div>
    ...
    if (EditFieldTypeMappings.hasOwnProperty(fieldType)) {
      ...
      fieldControl = <TextField
        ...
        value={value}
        errorMessage={`"${fieldType}"`}//{`${strings.UnsupportedFieldType} "${fieldType}"`}
        underlined
      />;
    }
  }
  ...
  if (fieldType != "Button") {
    _label = props.label || props.fieldSchema.Title;
    _description = props.description || props.fieldSchema.Description;
  }
