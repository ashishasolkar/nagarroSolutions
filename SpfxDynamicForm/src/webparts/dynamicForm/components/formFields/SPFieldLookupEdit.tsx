...
const SPFieldLookupEdit: React.SFC<ISPFormFieldProps> = (props) => {
    ...
    if (props.fieldSchema["DependerValue" as keyof typeof props.fieldSchema] != null) {
        let parentField = props.fieldSchema["DependerValue" as keyof typeof props.fieldSchema].Field;
        let parentValue = props.fieldSchema["DependerValue" as keyof typeof props.fieldSchema].Value;
        const splitArray = parentValue.split(';#');
        let values = splitArray.filter((item: any, idx: number) => (idx % 2 === 0));
        let parentId = Number(parentValue.split(";#")[0]);
        if (values.length > 0) {
            choices = choices.filter((x) => {
                let matches = values.filter((itm: any) => { return x.x[`${parentField}Id`] == itm; });
    ...
    if (props.fieldSchema.FieldType !== 'LookupMulti') {
        ...
        if (!props.required) { options = [{ key: 0, text: '' }].concat(options); }
        ...
        return <Dropdown
            ...
            options={options}
            selectedKey={value}
            onChanged={(item) => props.onEventTriggered(`${item.key};#${item.text}`)}
        />;
    } else {
        let values: any[] = [];
        if (props.value) {
        ...
        return <Dropdown
            ...
            multiSelect
            onChanged={(item) => props.onEventTriggered(getUpdatedValue(values, item))}
        />;
    }
};
