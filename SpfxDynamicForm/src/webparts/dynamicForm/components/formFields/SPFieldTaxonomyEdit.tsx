...
const SPFieldTaxonomyEdit: React.SFC<ISPFormFieldProps> = (props) => {
    ...
    return <TaxonomyPicker
        ...
        panelTitle={'select taxanomy'}//{strings.SelectTerm}
        label=""
        initialValues={terms}
        context={context}
        onChange={(items) => props.onEventTriggered(getUpdatedValue(items))}
        isTermSetSelectable={false} />;
};

function getUpdatedValue(terms: any[]) {
    let value = "";
